...
int getPublicIPAddress(struct STUNServer server, char *address) {
	...
	// Send the
	// printf("IP: %s - port: %d\n", inet_ntoa(remoteAddress->sin_addr), remoteAddress->sin_port);
	...
	if (response->type == htons(0x0101)) {
		...
		while (pointer < buffer + length) {
			...
			if (header->type == htons(XOR_MAPPED_ADDRESS_TYPE)) {
				...
				snprintf(address, 20, "%d.%d.%d.%d", (numAddress >> 24) & 0xFF, (numAddress >> 16) & 0xFF, (numAddress >> 8) & 0xFF, numAddress & 0xFF);
